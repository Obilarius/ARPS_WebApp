...
router.get("/children/:parentPathId", (req, res, next) => {
  ...
  pool.connect().then(() => {
    ...
    ps.input("id", sql.Int);
    ps.prepare("select * from fs.dirs where _parent_path_id = @id", function (
      err
    ) {
...
router.get("/userandgroupssid", async (req, res, next) => {
  try {
    ...
    let result = await conn.query(
      `SELECT SID, DisplayName as Name, 0 as isGroup FROM dbo.adusers UNION SELECT SID, Name, 1 as isGroup FROM dbo.adgroups`
    );
    res.send(result.recordset);
  } catch (err) {
    res.status(500).send("Fehler: " + err);
